<template>
  <GuiMenu class="flex w-full flex-col items-center justify-center gap-6 p-6">
    ...
    <div v-else-if="activeTool === 'text'" class="flex flex-col items-center justify-center gap-2">
      <GuiSlider class="backdrop-blur-none!" v-model="fontSize" name="Font size" image="/icons/text-size.svg" :min="1" :max="400" is-skewed :tooltip-format="(value) => `${Math.round(value)}px`" />
